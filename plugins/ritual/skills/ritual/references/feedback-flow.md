...
This is NOT `retry_recommendations` (failure recovery for generation) and NOT
...
1. **Read the current state.** `get_exploration` for the recommendation set;
 `list_recommendation_comments` (status `unresolved`) per recommendation the
 user cares about. Show the user what exists and what is still open.

2. **Post the user's feedback as comments.** One
 `add_recommendation_comment` per point, on the recommendation it is about:
...
4. **Start the revision.** `revise_recommendation_set` with the explicit
...
5. **Poll to ready.** `get_recommendation_revision` every ~10 seconds
...
7. **Apply.** `apply_recommendation_revision` with the revision id, its
...
- Use `retry_recommendations` or `accept_recommendations` anywhere in this
 flow.
